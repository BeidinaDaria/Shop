...
class Tools
{
    ...
    function login($name,$pass)
    {
        ...
        $link=Tools::connect();
        $query=$link->prepare("SELECT pass FROM `Customers` WHERE login=".$name.";");
        $res=$query->execute();
        if ($res){
            if ($res==$pass)
                return true;
            else {
                echo "<h3><span style='color:red;'>Not correct password!</span><h3/>";
                return false;
            }
        }
        else{
            echo "<h3><span style='color:red;'>User is not found!</span><h3/>";
            return false;
        }
    }
}
...
class Item
{
    ...
    function Draw()
    {
        echo "<div class='col-sm-3 col-md-3 col-lg-3 container' style='height:350px;margin:2px;' id='".$this->id."' onclick=>";
    ...
    function DrawForCart()
    {
        ...
        foreach($paths as $image){
            echo "<img src='".$image."'width='70px' class='col-sm-1 col-md-1 col-lg-1'/>";
        }
